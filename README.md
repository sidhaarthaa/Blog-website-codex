...
If Vercel logs still show old packages like `next@14` or `eslint@8`, it usually means cached dependencies from an older build are being reused.

1. In Vercel, redeploy with **"Use existing Build Cache" disabled**.
2. Confirm the project root is this repo root (where `package.json` and `next.config.js` live).
3. Keep Build Command as:
   - `node scripts/ensure-config.cjs && next build`
4. Verify install logs show `next@15.5.3` and `eslint@9.34.0`.
